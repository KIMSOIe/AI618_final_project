...
from functions.denoising_nonlinear import langevin_posterior_sampling,efficient_generalized_steps, stochastic_ddrm_sampling_aligned
...
import random
import cv2
def compute_sharpness_laplacian(img_tensor):
    """
    img_tensor: torch.Tensor [C, H, W], range [0,1]
    return: scalar sharpness (float)
    """
            ...
            for i in [-1]: #range(len(x)):
                for j in range(x[i].size(0)):
                    ...
                    if i == len(x)-1 or i == -1:
                        ...
                        #SSIM
                        pred_np = pred.clamp(0, 1).permute(1, 2, 0).cpu().numpy()  # [H, W, C]
                        gt_np = orig.clamp(0, 1).permute(1, 2, 0).cpu().numpy()
                        ssim = compute_ssim(gt_np, pred_np, data_range=1.0, channel_axis=-1)
                        avg_ssim += ssim

                        # Sharpness
                        sharpness = compute_sharpness_laplacian(pred.clamp(0, 1)) 
                        avg_sharpness += sharpness

            idx_so_far += y_0.shape[0]

            num_total = idx_so_far - idx_init
            pbar.set_description("PSNR: %.2f, SSIM: %.4f, Sharp: %.2f" % (avg_psnr / num_total, avg_ssim / num_total, avg_sharpness / num_total))

        avg_psnr = avg_psnr / (idx_so_far - idx_init)
        avg_ssim = avg_ssim / (idx_so_far - idx_init)
        avg_sharpness = avg_sharpness / (idx_so_far - idx_init)
        print("Total Average PSNR: %.2f" % avg_psnr)
        print("Total Average SSIM: %.2f" % avg_ssim)
        print("Total Average sharpness: %.2f" % avg_sharpness)
