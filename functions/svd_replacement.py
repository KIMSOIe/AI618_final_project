...
class Identity(H_functions):
    def __init__(self, channels, img_dim, device='cuda'):
        self.channels = channels
        self.img_dim = img_dim
        self.device = device
        # singulars = ones (no degradation)
        self._singulars = torch.ones(channels * img_dim * img_dim, device=device)

    def V(self, vec):   return vec.clone().reshape(vec.shape[0], -1)
    def Vt(self, vec):  return vec.clone().reshape(vec.shape[0], -1)
    def U(self, vec):   return vec.clone().reshape(vec.shape[0], -1)
    def Ut(self, vec):  return vec.clone().reshape(vec.shape[0], -1)
    def singulars(self):return self._singulars
    def add_zeros(self, vec): return vec.clone().reshape(vec.shape[0], -1)

    def H(self, x):
        # Identity: no blur/subsampling
        return x

    def adjoint(self, x):
        # Identity transpose = Identity
        return x
class PSFOperator(H_functions):
    def __init__(self, kernel_size=15, sigma=2.5, channels=3, image_size=256, device='cuda'):
        self.kernel_size = kernel_size
        self.sigma = sigma
        self.channels = channels
        self.image_size = image_size
        self.device = device
        self.kernel = self.create_gaussian_kernel(kernel_size, sigma).to(device)
        self._singulars = torch.ones(channels * image_size * image_size, device=device)
    ...
    def H(self, x):
        ...
        return F.conv2d(x, self.kernel, padding=padding, groups=self.channels)
